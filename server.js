...
app.put("/TaskCategories/:CategoryID", (req, res) => {
  ...
  db.query("UPDATE TaskCategories SET CategoryName = ? WHERE CategoryID = ?", [CategoryName, id], (err, results) => {
    if (err) throw err;
    res.json(results);
  });
});
...
app.delete("/TaskCategories/:CategoryID", (req, res) => {
  const { id } = req.params;
  db.query("DELETE FROM TaskCategories WHERE CategoryID = ?", [id], (err, results) => {
    if (err) throw err;
    res.json(results);
  });
});
...
app.put("/TaskStatus/:id", (req, res) => {
  ...
  db.query("UPDATE TaskStatus SET StatusName = ? WHERE StatusID = ?", [StatusName, id], (err, results) => {
    if (err) throw err;
    res.json(results);
  });
});

app.delete("/TaskStatus/:id", (req, res) => {
  const { id } = req.params;
  db.query("DELETE FROM TaskStatus WHERE StatusID = ?", [id], (err, results) => {
    if (err) throw err;
    res.json(results);
  });
});
...
app.put("/Tasks/:id", (req, res) => {
  ...
  db.query("UPDATE Tasks SET Title = ?, Description = ?, DueDate = ?, UserID = ?, CategoryID = ?, StatusID = ? WHERE TaskID = ?",
    [Title, Description, DueDate, UserID, CategoryID, StatusID, id], (err, results) => {
      if (err) throw err;
      res.json(results);
    });
});
...
app.delete("/Tasks/:id", (req, res) => {
  const { id } = req.params;
  db.query("DELETE FROM Tasks WHERE TaskID = ?", [id], (err, results) => {
    if (err) throw err;
    res.json(results);
  });
});
